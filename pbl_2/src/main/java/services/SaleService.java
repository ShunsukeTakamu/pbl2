...
public class SaleService {
    ...
    public int insert(Sale s) {
        ...
        try (Connection con = Db.open(); PreparedStatement ps = con.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
    ...
    public Map<String, Integer> getSalesByAccount() {
        ...
        try (Connection con = Db.open(); PreparedStatement ps = con.prepareStatement(sql); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
    ...
    public int getTotalSales() {
        ...
        int total = 0;
        try (Connection con = Db.open(); Statement stmt = con.createStatement(); ResultSet rs = stmt.executeQuery(sql)) {
            if (rs.next()) {
                total = rs.getInt(1);
            }
        } catch (Exception e) {
    ...
    public Map<String, Integer> getSalesByYear() {
        ...
        for (String key : yearMap.keySet()) keys.add(Integer.parseInt(key));
        if (keys.isEmpty()) return yearMap;
